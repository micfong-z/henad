...
description: Six fixes from the runner review, covering frame pacing, a late readback and CLI ranges.
icon: material/note-text-outline
status: ai-generated
model: claude-opus-5 (Claude Code)
issue: none
state: complete — all six fixed, each with a regression test, verified in the native app
