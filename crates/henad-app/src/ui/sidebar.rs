    ...
    egui::SidePanel::left("sidebar")
        ...
        .show(ctx, |ui| {
            ...
            let is_gpu = app
                .sim_thread
                .as_ref()
                .is_some_and(|t| t.gpu_stats().is_some());
            ...
            if is_gpu {
                ...
                // TPS controls
                {
                    let uncapped_changed =
                        ui.checkbox(&mut app.uncapped, "Unlimited TPS").changed();
...
fn gpu_batching_controls(ui: &mut egui::Ui, app: &mut HenadApp) {
    ...
    if app.gpu_adaptive {
        ...
        ui.add_enabled(
            false,
            egui::Slider::new(&mut live_batch_size, 1..=MAX_BATCH_SIZE)
