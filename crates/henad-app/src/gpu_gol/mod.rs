...
impl GpuGolResources {
    ...
    fn dispatch_step(&mut self, encoder: &mut wgpu::CommandEncoder) {
...
/// What to reseed the grid with; requested from the UI and applied on the next `prepare` call.
#[derive(Clone, Copy)]
enum ReseedKind {
    Patterns,
...
pub struct GpuGolCallback {
    paused: bool,
    reseed: Option<ReseedKind>,
}

impl CallbackTrait for GpuGolCallback {
    fn prepare(
        &self,
        _device: &wgpu::Device,
        queue: &wgpu::Queue,
        ...
        let resources: &mut GpuGolResources = callback_resources
            .get_mut()
            .expect("GpuGolResources must be inserted before the GoL panel runs");

        if let Some(kind) = self.reseed {
            let cells = match kind {
                ReseedKind::Patterns => seed_patterns(resources.width, resources.height),
                ReseedKind::Random { seed, density } => {
                    seed_random(resources.width, resources.height, density, seed)
                }
            };
            resources.reseed(queue, &cells);
        }

        if !self.paused {
            resources.dispatch_step(egui_encoder);
        }
        resources.dispatch_display(egui_encoder);
    ...
    fn paint(
        ...
        let resources: &GpuGolResources = callback_resources
            .get()
            .expect("GpuGolResources must be inserted before the GoL panel runs");
...
/// Opens a window showing the live GPU Game of Life, with pause and reseed controls.
pub fn gpu_gol_panel(ctx: &egui::Context) {
    ...
    egui::Window::new("GPU Game of Life").show(ctx, |ui| {
        ...
        ui.horizontal(|ui| {
            if ui.button("Reseed: patterns").clicked() {
                reseed = Some(ReseedKind::Patterns);
            }
            if ui.button("Reseed: random").clicked() {
                let seed = ctx.input(|i| i.time.to_bits()).max(1);
                reseed = Some(ReseedKind::Random { seed, density: 0.3 });
            }
        });
        ...
        ui.painter().add(egui_wgpu::Callback::new_paint_callback(
            rect,
            GpuGolCallback { paused, reseed },
        ));
    });

    ctx.data_mut(|d| d.insert_temp(paused_id, paused));
}
...
mod tests {
    ...
    fn gpu_matches_cpu_reference() {
        ...
        let mut resources = GpuGolResources::new(
            &device,
            &queue,
        ...
        for _ in 0..ticks {
            let mut encoder =
                device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
            resources.dispatch_step(&mut encoder);
            queue.submit(Some(encoder.finish()));
        ...
        let gpu_state = read_buffer(
            &device,
            &queue,
            resources.current_buffer(),
            (width * height) as usize,
        );

        assert_eq!(
            gpu_state, cpu_state,
            "GPU state after {ticks} ticks must match the CPU reference"
        );
    }
