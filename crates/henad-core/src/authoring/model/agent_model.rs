...
pub trait AgentModel: Send + Sync + 'static {
    ...
    ///
    /// Takes what `init` takes plus the field, an action being a setup step the user asks for
    /// mid run. `rng` is a stream of its own, so a press leaves the tick's draws where they were.
    fn act(
        _action: usize,
