...
use crate::virus_network::{EDGE_PALETTE, PALETTE, VirusNetwork};
...
/// Keeps the nodes' streams apart from the seed the CPU `init` draws from.
const RNG_SALT: u64 = 0x5EED_5EED_5EED_5EED;

// Listed in the Model panel.
pub const BUFFERS: &[&str] = &["state", "timer", "rng", "pos", "color", "edges", "row_start", "entries"];
pub const PASSES: &[&str] = &["step"];
...
/// A compute pass with one bind group per `state` side, indexed by the side holding the current tick.
struct Pass {
    label: &'static str,
...
impl Pass {
    fn encode(
        ...
        pass.set_bind_group(0, &self.binds[side], &[]);
...
pub struct GpuVirusNetwork {
    ...
    num_nodes: u32,
    tick: u64,
    /// Index of the `state` side holding the current tick.
    side: usize,
...
impl GpuVirusNetwork {
    ...
    fn from_cpu(
        ...
        let state = [
            storage_buffer(device, &format!("{ID}_state_a"), n),
            storage_buffer(device, &format!("{ID}_state_b"), n),
        ];
        queue.write_buffer(&state[0], 0, bytemuck::cast_slice(&states));
        ...
        let step = Pass {
            ...
            binds: [0, 1].map(|side| {
                bind_group(
                    device,
                    &format!("{ID}_step_bind{side}"),
                    &step_layout,
                    &[
                        state[side].as_entire_binding(),
                        state[1 - side].as_entire_binding(),
        ...
        let recolor_nodes = Pass {
            ...
            binds: [0, 1].map(|side| {
                bind_group(
                    device,
                    &format!("{ID}_recolor_nodes_bind{side}"),
                    &node_color_layout,
                    &[
                        state[side].as_entire_binding(),
        ...
        let recolor_edges = Pass {
            ...
            binds: [0, 1].map(|side| {
                bind_group(
                    device,
                    &format!("{ID}_recolor_edges_bind{side}"),
                    &edge_color_layout,
                    &[
                        state[side].as_entire_binding(),
...
impl GpuSimState for GpuVirusNetwork {
    fn encode_steps(&mut self, encoder: &mut wgpu::CommandEncoder, count: u32, timestamps: Option<&wgpu::QuerySet>) {
        for i in 0..count {
            let stamps =
                timestamps
                    .filter(|_| i == 0 || i == count - 1)
                    .map(|query_set| wgpu::ComputePassTimestampWrites {
                        query_set,
                        beginning_of_pass_write_index: (i == 0).then_some(0),
                        end_of_pass_write_index: (i == count - 1).then_some(1),
                    });
            self.step.encode(encoder, self.side, stamps);
    ...
    fn encode_snapshot_passes(&mut self, encoder: &mut wgpu::CommandEncoder) {
        ...
        self.recolor_nodes.encode(encoder, self.side, None);
        self.recolor_edges.encode(encoder, self.side, None);
...
fn demand_for(num_nodes: u32, num_edges: u32) -> Demand {
    ...
    demand.push_sides(&format!("{ID}_state"), n, true);
...
impl GpuVirusNetwork {
    ...
    pub(crate) fn read_states(&self) -> Vec<u32> {
        henad_compute::gpu::primitives::readback::read_words(&self.device, &self.queue, &self.state[self.side])
