...
function ants_field(width, height, layer)
    a, b, m = layer == "to_food" ? (7, 13, 97) : (11, 5, 89)
    return [((a * x + b * y) % m + 1) / (m + 1) for x in 0:(width - 1), y in 0:(height - 1)]
end

# Cells alive at the start, as `(x, y)` offsets placed at the origin of a 64 by 64 torus.
const GAME_OF_LIFE = Dict(
    "glider" => [(1, 0), (2, 1), (0, 2), (1, 2), (2, 2)],
    "r-pentomino" => [(1, 0), (2, 0), (0, 1), (1, 1), (1, 2)],
)
const GAME_OF_LIFE_STEPS = Dict("glider" => 101, "r-pentomino" => 500)
