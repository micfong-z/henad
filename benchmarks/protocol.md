...
Every engine in the comparison is driven through one program with the same interface, so
`scripts/compare_bench.py` needs no per-engine knowledge beyond how to start it.
Henad's side of the contract is `henad-cli --json`.
...
Parameters an engine does not have are an error, not a silent default: a mismatched parameter set
means the two runs are not the same model.
...
```json
{"kind":"info","engine":"mesa","engine_version":"3.3.0","variant":"default","threads":1}
...
`rep` comes once per timed rep, as soon as that rep finishes, so a run killed by a timeout still
...
The driver computes its own statistics from the `rep` lines and uses `summary` only for provenance,
so no engine's own arithmetic reaches a published table.

## What is timed

The step loop, and nothing else.
Construction, initial population, warm-up and teardown all sit outside the window, and the timer is
the engine's own monotonic clock rather than the wall time of the process.
An engine that compiles or JITs runs one full untimed rep first.

## Validate mode

`--validate` runs a fixed scenario and writes the state it reaches, in the fixture format the
matching document under `crates/henad-models/tests/fixtures/docs/` gives.
`scripts/validate_ports.py` compares that against Henad and refuses to time a port that disagrees.
