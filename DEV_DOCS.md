...
## 7. Rough edges and open questions

### 7.1 The adaptive controller regulates *encode* cost, not GPU cost, at the sizes measured

**What the code measures.** `step_batch` starts a timer, encodes `batch_size` compute passes, calls
`queue.submit()`, and stops the timer the instant `submit()` returns
(`gpu/sim_thread.rs:340-362`). That elapsed wall-clock time, divided by `batch_size`, is the EMA
input; `next_batch_size(ema, target_ms)` then picks a batch size such that
`batch_size × ema ≈ target_ms`.

**What the comments say.** The code is more self-aware than one might fear, and the doc at
`gpu/sim_thread.rs:326-339` should be read in full. It states plainly that this is
"deliberately wall-clock CPU time, not a GPU timestamp query", names the assumption it rests on —
that on a continuously-busy queue the *rate* at which `submit()` can be issued gets backpressured by
how fast the GPU drains — and then says:

> "…is plausible but has **not** been empirically verified. If it doesn't hold on some
> backend/platform (e.g. `submit()` returns immediately regardless of queue depth), this instead
> mostly measures CPU-side dispatch-recording cost … so `time_per_step` would stay roughly constant
> regardless of true GPU load, and the controller would regulate encode cost rather than the
> GPU-stutter problem it's meant to solve."

**So the comment is not wrong about the mechanism — it is stale about the evidence.** The open
question it flags has since been answered, and answered *against* the assumption. The residual
inaccuracy is in the **module-level** doc, `gpu/sim_thread.rs:58`, which still calls wall-clock
"a proxy for GPU cost". At the measured operating point, it is not.

**The measurement.** On a 1024² grid, `TimestampQuery` reported ≈ **0.058 ms/step** of true GPU
time, while the controller's implied EMA (recoverable as `target_ms / live_batch_size`) was
≈ **0.167 ms/step** — a **≈ 2.9×** gap. The remainder is CPU-side encode-and-submit cost.

**What that gap actually implies** (this is the part worth putting in a paper, and it is a
mechanism claim, so state it carefully):

- At 1024², 0.167 ms/step of CPU cost buys 0.058 ms/step of GPU work. A batch of ~48 steps (which is
  what an 8 ms budget yields at that EMA) therefore occupies the GPU for ~2.8 ms out of every ~8 ms.
  **The GPU backend is submission/encode-bound at 1 M cells, not GPU-bound** — the device is idle
  roughly 65 % of the time. Reported TPS at this grid size is a measure of how fast wgpu can build
  Metal compute encoders, not of how fast the GPU can run Life.
- The direct cause is the one-pass-per-step structure forced by wgpu's inter-pass-only barriers
  (§6.7). Each step is a fresh `begin_compute_pass`, and a compute encoder is not a cheap object.
- **The controller therefore does not currently do what it is for.** Its stated purpose is to bound
  the *GPU-side* duration of a submission so a long batch cannot block egui's render pass on the
  shared queue. If its input is dominated by a roughly *constant* per-step encode cost, then
  `batch_size ≈ target_ms / const` — i.e. it degenerates to a fixed batch size that does not respond
  to GPU load at all.
- **It nonetheless works in practice, and there is a plausible reason why.** As the grid grows, GPU
  cost per step rises (∝ cells) while encode cost per step does not. Once GPU time per step exceeds
  encode time per step, the queue backs up, and *if* `submit()` backpressures under queue depth, the
  wall-clock signal starts tracking GPU drain rate — exactly in the regime where stutter happens.
  In the encode-bound regime (small grids), an oversized batch is harmless anyway, because the GPU
  work in it is short. So the design may be right in the regime that matters and merely
  *inert-but-harmless* in the regime that doesn't. FPS staying > 110 is consistent with that. It is
  also consistent with "we simply never generated a batch long enough to stutter".

**The experiment that settles it** (and it is cheap): run a 4096² grid in adaptive mode and record
`gpu_us_per_step × live_batch_size`.
- If that product tracks `target_ms` → `submit()` does backpressure, the assumption holds in the
  regime that matters, and the design is defensible as written (with the comment updated).
- If the product blows far past `target_ms` while `live_batch_size` stays pinned near its
  small-grid value → there is no backpressure, the controller is regulating encode cost only, and
  the honest fix is to feed the controller `gpu_us_per_step` from `TimestampQuery` (promoting it
  from diagnostic to load-bearing) or to measure a submission-completion timestamp instead of
  `submit()` return.

Until that is run, **no claim about the controller regulating GPU time is defensible**, and the
module doc at `gpu/sim_thread.rs:58` overstates what is known.

**Two smaller contaminants in the same signal:**

1. On snapshot iterations (every 16 ms) the timed window *also* includes
   `encode_snapshot_passes` (`gpu/sim_thread.rs:356-358` sits inside the timer), so roughly one
   batch in every 16 ms gets a per-step cost inflated by display+reduce encode work that is not
   attributable to steps. With α = 0.25 that sample carries real weight.
2. `gpu_us_per_step` itself was observed swinging **58 → 91 µs across two runs on an identical
   grid** — a 57 % spread that the workload does not justify (the shader is deterministic and the
   grid is the same size). Candidates: GPU clock/power state on the M4 Pro, contention with egui's
   own submissions on the shared queue, or the timestamp period being read once at construction
   (`timing.rs:81`) while the effective clock varies. Worth pinning down before any number from this
   readout goes into a paper — a diagnostic with 57 % run-to-run variance is not yet a measurement
   instrument.

### 7.2 `ModelState` was an agent's addition beyond the approved design

The approved design had the registry factory returning `Box<dyn SimState>`. What landed is the
tagged enum `ModelState::{Cpu, Gpu}` (`registry.rs:17`).

The justification is sound (see §6.3 — without the tag, `HenadApp` cannot tell which runner to spawn
without downcasting, and `GpuSimState` being a subtrait of `SimState` means the information is
genuinely destroyed by erasure to the supertrait). But it *is* a scope addition made by a delegated
agent, and it has a cost worth naming: **the registry's return type now knows about GPU-ness**. A
third backend (a distributed runner, an FPGA, a second GPU path) means a third variant, and every
`match` on `ModelState` — currently `lib.rs:175-203` plus the tests — grows an arm. The enum is a
closed set in a codebase that otherwise leans on open traits.

If that ever becomes painful, the shape to move to is probably "the factory returns something that
knows how to spawn its own runner" rather than "the factory returns a value the caller must
classify".

### 7.3 The GPU grid-size sliders go far past what the device can allocate

This one is not in anyone's notes and is a real, reachable crash.

`GpuGameOfLifeModel::param_descriptors` (`gpu_game_of_life/mod.rs:112-125`) declares:
...
The device the app actually creates inherits **egui's default limits** (`main.rs:15-24` decorates
`WgpuSetupCreateNew::default()`, which sets `max_texture_dimension_2d: 8192` and otherwise takes
`wgpu::Limits::default()`). Those defaults are:

| Limit | Value |
| --- | --- |
| `max_texture_dimension_2d` | 8 192 |
| `max_storage_buffer_binding_size` | 128 MiB |
| `max_buffer_size` | 256 MiB |

The model allocates one `u32` per cell in each of two storage buffers, plus an
`RGBA8` display texture of exactly `width × height`. So:

- Any dimension **> 8 192** exceeds `max_texture_dimension_2d` — the display texture cannot be
  created. The slider allows up to 16 384.
- `width × height` **> 33 554 432 cells** (128 MiB ÷ 4 B) exceeds `max_storage_buffer_binding_size`.
  That is a **5 792 × 5 792** square grid. A 8192 × 8192 grid — well inside the slider range —
  needs 256 MiB per buffer, which also exceeds `max_buffer_size`.

Nothing clamps or pre-flights this: `GpuGameOfLifeState::new` (`:185`) reads the params, does
`.max(1)`, and calls `create_buffer` / `create_texture` directly. The failure mode is a wgpu
validation error whose default handler panics — and it happens on the **UI thread**, inside
`reset_simulation`, before the sim thread is even spawned.

This is also the concrete form of the "bit-packing becomes mandatory past ~33.5 M cells" note in
§8: 33.5 M cells is not a distant ceiling, it is *below the slider's own maximum*, and the engine's
stated target is 10 M+ agents.

Cheapest honest fix: read `device.limits()` in `GpuGameOfLifeModel::new`, and derive the descriptor
maxima from it rather than hard-coding 16 384. (Not done here — this document changes no source.)

### 7.4 The `fragile-send-sync-non-atomic-wasm` feature is a wasm-typecheck workaround

`Cargo.toml:24-37` enables `wgpu/fragile-send-sync-non-atomic-wasm` **workspace-wide**, with a
comment that is accurate about the why:

> GPU models are native-only for now, but `SimState: Send`, so without this the GPU model's
> `SimState` impl would not typecheck for `wasm32-unknown-unknown` (which CI checks) and would have
> to be `#[cfg]`-ed out of existence instead.

The mechanism: `SimState: Send` (`henad-core/src/model.rs:19`). `GpuSimState: SimState`
(`gpu/sim_thread.rs:76`). `GpuGameOfLifeState` holds `wgpu::Device`, `wgpu::Queue`,
`wgpu::Buffer`, `wgpu::BindGroup`, etc. On `wasm32` those are `!Send` by default (they wrap JS
objects, which are thread-affine). And `ModelState::Gpu(Box<dyn GpuSimState>)` (`registry.rs:19`) is
**not** `#[cfg]`-gated, so the variant — and therefore the `Send` obligation — exists in the wasm
build even though no GPU model is ever constructed there.

The tradeoff, stated plainly:

- **What we bought:** one feature flag, and `henad-models` compiles for wasm unchanged. No `#[cfg]`
  spidering through the registry, the model module, `ModelState`, and their tests.
- **What we paid:** a wgpu feature whose name literally says "fragile" is on for the *whole
  workspace*, including the web build, permanently. It is sound today only because there are no
  threads on wasm, so the handles are never actually sent. That soundness argument is a *property of
  the current app*, not of the type system — if wasm ever gets threads (which is exactly what a
  future WebGPU + `SharedArrayBuffer` build would want, see §8), the flag becomes an unsound lie
  rather than a harmless one.
- **The alternative:** `#[cfg(not(target_arch = "wasm32"))]` on the `gpu_game_of_life` module, the
  `ModelState::Gpu` variant, and the registry's GPU arm. More `#[cfg]`s, but no global feature flag,
  and the wasm build genuinely doesn't contain the type.

The current choice is defensible; it is not free, and the note in `Cargo.toml` should be revisited
the moment wasm threading enters the picture.

### 7.5 `StatsHistory` is sampled per-*snapshot*, not per-tick — and its x-axis is mislabelled

`view.rs:54` says "recorded every tick by the model" and `view.rs:76` says "Called once per tick by
the model". Both are **factually wrong** about the current code:

- Nothing in `henad-core`, `henad-compute` or `henad-models` calls `StatsHistory::push` at all.
- The **app** calls it, once per snapshot it polls, at `henad-app/src/lib.rs:234-237`.

The consequence is not cosmetic. Snapshots are throttled to ~16 ms (`sim_thread.rs:175`,
`gpu/sim_thread.rs:175`), so:

- CPU model at 1000 uncapped TPS → history records ~60 of every 1000 ticks.
- GPU model → each snapshot can be **thousands of ticks** apart.

And the chart plots those samples against `oldest_tick + j` (`sidebar.rs:404`), where `oldest_tick`
is `write_count - capacity` (`view.rs:129`) — i.e. a **snapshot counter, presented as a tick
number**. For the GPU Game of Life the x-axis is therefore off by a factor of the batch rate, and
for an uncapped CPU model by a factor of ~16. The "History length" slider is likewise in snapshots,
not ticks.

Nothing here is a crash; it is a chart that quietly lies about its own x-axis, in a project whose
output is meant to be figures in a paper. That makes it worth more than its size suggests.

### 7.6 CPU stats are computed every tick, sequentially; GPU stats are not

`GridModelState::step` recomputes `M::stats(&self.grid)` on **every tick**
(`grid_engine.rs:63`), and `M::stats` is a sequential full-grid scan (`sir.rs:99`,
`game_of_life.rs:65`). Boids does the same — three sequential passes over the velocity arrays every
step (`boids/step.rs:14-22`).

So on the CPU side, every tick pays an O(n) *sequential* pass on top of an O(n) *parallel* one,
whether or not anyone will look at the result (recall snapshots publish at most every 16 ms). At
1024² that is 1 M elements per tick; at the engine's stated 10 M+ target it is the sequential
fraction that caps the speedup — a textbook Amdahl term sitting in the hot loop, in a project whose
entire thesis is parallel throughput.

The GPU model, by contrast, computes its stat **only at display cadence** and does it in parallel
(`gpu/sim_thread.rs:344`, `reduce.wgsl`). This is an *asymmetry in the benchmark*, not just in the
code: a CPU-vs-GPU comparison on Game of Life is currently comparing "step + count every tick"
against "step every tick, count 60×/second". For the fair-comparison rule the paper commits to, the
CPU side should either compute stats at snapshot cadence too, or parallelise the count (a rayon
`par_iter().filter().count()` is a two-line change), or both. Worth fixing *before* generating any
comparative number.

### 7.7 Smaller frictions

- **`GpuSimState: SimState` forces a vestigial `step()`.** `gpu_game_of_life/mod.rs:458` implements
  `SimState::step` as one encoder + one pass + one `submit()` per tick — correct but "slow", as its
  own doc says (`:454-457`), and **nothing calls it**. `GpuSimThread` always goes through
  `encode_steps`. It exists purely to honour the supertrait. It is dead code that typechecks, and
  it is a live trap: it looks like a supported path.
- **Test-only struct fields.** `buffer_a` / `buffer_b` are `#[cfg(test)]` fields on
  `GpuGameOfLifeState` (`:157-160`), because production code never needs the raw handles (the bind
  groups hold their own refs). The struct therefore has a different shape in test and release
  builds. It works, but "the type under test is not the type that ships" is a smell, and it will
  have to be undone the moment in-place reseed or resize lands.
- **The palette exists twice.** `game_of_life.rs:16` (Rust) and `gpu_game_of_life/display.wgsl:6-7`
  (WGSL constants). `game_of_life.rs:11-15` admits this in a comment. A palette change silently
  desynchronises the two backends' *appearance* while all correctness tests still pass (they test
  counts, not colours).
- **The toroidal-index rule exists six times.** `grid.rs:80` (unused helper), `grid_engine.rs:137`
  (moore/native), `:165` (moore/wasm), `:206`/`:232` (von Neumann ×2), `step.wgsl:17`, and again in
  the GPU test's `cpu_reference_step` (`gpu_game_of_life/mod.rs:702`). Every one is correct today.
  There is no single place to fix it if one ever isn't.
- **`Grid2D::moore_neighbors` / `von_neumann_neighbors` are dead outside tests.** The engine
  re-derives the arithmetic inline for hoisting reasons. So the *tested* neighbour code and the
  *executed* neighbour code are different code. If you ever change the wrap convention, the tests
  will not catch you.
- **`TopologyHint` is dead metadata.** Declared (`topology.rs:3`), required of every `Model`
  (`model.rs:14`), stored on every `ModelEntry` (`registry.rs:37`), read by nobody. `Network` and
  `NonSpatial` have no implementors. The viewport deliberately branches on the snapshot variant
  instead (`viewport.rs:81`).
- **`SimState::set_param`'s `bool` return is discarded** by both runners (`sim_thread.rs:147`,
  `gpu/sim_thread.rs:263`). So a model *cannot* signal "that parameter needs a Reset" — the GPU
  model returns `false` for everything (`gpu_game_of_life/mod.rs:488`) and the slider just silently
  does nothing until the user presses Reset. There is a wired-up channel here that nobody listens
  to.
- **Native and wasm grid stepping are not behaviourally identical.** Per-row reseeding on native
  (`grid_engine.rs:134`), one sequential rng on wasm (`:161`). SIR trajectories differ between
  builds. CLAUDE.md asserts they "must stay behaviorally identical"; they don't. (Determinism
  *within* each build is preserved, which is the property that actually matters for reproducibility
  — but the claim as written is false.)
- **The GPU model's grid-size params have a different range from the CPU model's** — 1..=16 384
  (`gpu_game_of_life/mod.rs:114`) vs 1..=10 000 (`grid_engine.rs:50`) — while the comment at
  `gpu_game_of_life/mod.rs:47` claims the descriptors match "so this model is a drop-in comparison
  against the CPU one". The *indices* match; the *ranges* don't. See also §7.3.
- **`gol_blinker_period_2` (`game_of_life.rs:122`) does not test a blinker.** It sets up a 5×5 grid,
  comments "We need to step the internal grid, so let's create a fresh state and manually init
  instead. Just verify tick advancement", and then asserts only `tick == tick0 + 1`. The name
  promises a period-2 oscillator check that the body does not perform. (The GPU model *does* have a
  real blinker test, `:771`.)
- **`ui/stats.rs` is a two-line placeholder** while the stats chart actually lives in `mod stats`
  inside `sidebar.rs:376`.
- **"Reset" and "Load Model" are the same button.** `sidebar.rs:97-105` — both call
  `app.reset_simulation()`.
- **GPU tests silently skip without an adapter.** Every test in `gpu_game_of_life`'s two test
  modules `return`s early with a `log::warn!` if `request_adapter` fails. A green `cargo test` on a
  headless machine proves nothing about the GPU path.
